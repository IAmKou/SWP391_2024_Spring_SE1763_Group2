...
public class SendFeedbackServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        int post_id = 1;
        int user_id = 2;
        int number_of_star = Integer.parseInt(request.getParameter("star"));
        String comment = request.getParameter("comment");
        FeedbackDAO feedbackDao = new FeedbackDAO();
        
        feedbackDao.addFeedback(post_id,user_id,number_of_star,comment);
        
        
        request.setAttribute("mess", "submit feedback success");
        request.getRequestDispatcher("sendFeedback.jsp").forward(request, response);
    }
